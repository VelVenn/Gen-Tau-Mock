...
|   ├── udp-video-streamer-fast.js # UDP 视频流服务（不限制编码端发送速度）
...
| 字段 | 类型 | 偏移 | 长度 | 说明 |
|------|------|------|------|------|
| 帧编号 | uint16 (LE) | 0 | 2 bytes | 当前帧序号（0-65535 循环） |
| 分片序号 | uint16 (LE) | 2 | 2 bytes | 当前分片在帧中的序号（从 0 开始） |
| 总字节数 | uint32 (LE) | 4 | 4 bytes | 该帧的总字节数 |
...
- **最大分片大小**: 1392 字节（视频数据部分）
- **完整包大小**: 1400 字节（8字节包头 + 1392字节数据）
- **分片逻辑**: 
  - 如果一帧 > 1392 字节，则分割成多个包
  - 最后一个包可能 < 1392 字节
