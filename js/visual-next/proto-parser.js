...
class ProtoParser {
        ...
        for (let i = startIdx; i < endIdx; i++) {
            ...
            if (msgMatch) {
                ...
                if (freqMatch && !config.messageDefaultFrequencies[currentMessage]) {
                    config.messageDefaultFrequencies[currentMessage] = parseFloat(freqMatch[1]);
                } else if (cleanedDescription.includes('触发') && typeof config.messageDefaultFrequencies[currentMessage] === 'undefined') {
                ...
                const displayName = config.messageDisplayNames[currentMessage] || config.messageDisplayNames[cleanedDescription] || cleanedDescription || currentMessage;
    ...
    parseFieldValues(messageType, data) {
        ...
        for (const [fieldName, value] of Object.entries(normalizedData)) {
            ...
            if (messageType === 'DeployModeStatusSync' && realFieldName === 'status') {
                mappingKey = 'deploy_mode_status';
            } else if (messageType === 'TechCoreMotionStateSync' && realFieldName === 'status') {
                mappingKey = 'core_status';
            }

            const statusMapping = config.statusMappings[mappingKey] || config.statusMappings[fieldName];
            if (statusMapping && Array.isArray(statusMapping)) {
                ...
                if (fieldName.includes('button') || fieldName.includes('down')) {
                    ...
                    if (match) {
                        display = value ? (match[4]?.trim() || '是') : (match[2]?.trim() || '否');
                    } else {
                        display = value ? '是' : '否';
                    }
                } else {
    ...
    findEnumComment(metadata, fieldName) {
        ...
        if (metadata.fields[fieldName]) {
            const desc = metadata.fields[fieldName].description || metadata.fields[fieldName].comment;
