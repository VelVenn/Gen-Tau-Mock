...
class HTTPServer {
  ...
  start() {
    ...
    this.httpServer.listen(this.httpPort, this.host, () => {
      console.log(
        `✅ Web 可视化界面已启动 - http://${this.host}:${this.httpPort}`,
      );
        ...
        this.mqttHandler.publish(
          ...
          (err, size) => {
            if (err) {
              ...
              console.log(
                `📤 手动发送下行消息 - 类型: ${messageType}, 大小: ${size} 字节`,
              );
              res.writeHead(200, { "Content-Type": "application/json" });
              res.end(
                JSON.stringify({
  ...
  handleAutoPublish(req, res) {
    ...
    req.on("end", () => {
      try {
        const { messageType, enabled, intervalMs, topic, data } =
          JSON.parse(body);
        ...
        if (enabled) {
          this.mqttHandler.startAutoPublish(
            messageType,
            intervalMs,
            topic,
            convertedData,
        ...
        res.end(
          JSON.stringify({
            ...
            intervalMs:
              this.mqttHandler.autoPublishers[messageType]?.intervalMs || 0,
          }),
        );
      } catch (error) {
        res.writeHead(500, { "Content-Type": "application/json" });
