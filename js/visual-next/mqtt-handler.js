  ...
  async start() {
    return new Promise((resolve, reject) => {
          ...
          if (topic.includes(msgName) || topic === msgName) {
            try {
              ...
              if (this.receivedMessages.length > this.maxHistorySize) {
                this.receivedMessages = this.receivedMessages.slice(
                  0,
                  this.maxHistorySize,
                );
              }
              ...
              console.log(
                `📥 收到上行消息 - 客户端: ${client.id}, 类型: ${msgName}`,
              );
            } catch (err) {
      ...
      this.mqttServer.listen(this.mqttPort, this.host, () => {
        console.log(
          `✅ MQTT 服务已启动 - mqtt://${this.host}:${this.mqttPort}`,
        );
        resolve();
      });
    });
    ...
    const timer = setInterval(() => {
      this.publish(messageType, publishTopic, template, (err, size) => {
        if (err) {
          ...
          console.log(
            `📤 自动发送下行消息 - 类型: ${messageType}, 大小: ${size} 字节`,
          );
        }
      });
    }, ms);

    this.autoPublishers[messageType] = {
      timer,
      intervalMs: ms,
      topic: publishTopic,
    };
