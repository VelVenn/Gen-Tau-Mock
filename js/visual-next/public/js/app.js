...
async function loadMessages() {
    try {
        ...
        document.getElementById('uplinkMessages').innerHTML = '<p class="loading-text" style="color:red">加载配置失败，请确保后台运行正常</p>';
    }
}
...
function renderHistory() {
    ...
    uplinkHistory.forEach(msg => {
        ...
        const timeStr = timeObj.toLocaleTimeString() + '.' + timeObj.getMilliseconds().toString().padStart(3, '0');
...
window.sendDownlinkMessage = function(messageName) {
    ...
    Object.entries(meta.fields).forEach(([fieldName, fieldMeta]) => {
        ...
        } else if (fieldMeta.type === 'int32' || fieldMeta.type === 'uint32') {
